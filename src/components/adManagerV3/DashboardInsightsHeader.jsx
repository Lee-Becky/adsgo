...
const Tooltip = ({ accounts, title, platform, buttonRef }) => {
  ...
  return createPortal(
    <div 
      className="absolute bg-white border border-neutral-200 rounded-lg shadow-lg p-3 z-[999999] min-w-[200px]"
      style={{ top: position.top, left: position.left }}
    >
      <div className="text-xs font-bold text-neutral-800 mb-2 pb-1 border-b border-neutral-100">{title}</div>
      {accounts.map((account, index) => (
        <div key={index} className="text-xs text-neutral-600 py-1 flex justify-between">
...
const DashboardInsightsHeader = ({
  ...
  return (
    <div className="bg-white border border-neutral-200 rounded-2xl shadow-sm overflow-hidden relative z-10">
      {/* Card Header */}
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2">
              <Eye size={16} className="text-info-500" />
              <h2 className="text-sm font-black text-neutral-800 tracking-wide">Optimization Overview</h2>
            </div>
            ...
            <div className="inline-flex items-center bg-neutral-100 rounded-lg p-1">
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md transition-all ${
                  activeTab === 'all' 
                    ? 'bg-white text-info-600 shadow-sm font-bold' 
              ...
              >
                <svg 
                  ...
                  className={`w-4 h-4 ${activeTab === 'all' ? 'text-info-600' : 'text-neutral-600'}`}
                >
                  <path 
              ...
              >
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md transition-all ${
                    activeTab === 'meta' 
                      ? 'bg-white text-info-700 shadow-sm font-bold' 
                      : 'text-neutral-600 hover:text-neutral-800'
                  }`}
                >
          ...
          <div className="flex items-center gap-3">
            ...
            {activeTab !== 'google' && (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-neutral-200 rounded-lg hover:bg-neutral-50 transition-all shadow-sm text-neutral-600 hover:text-neutral-800 group"
              >
