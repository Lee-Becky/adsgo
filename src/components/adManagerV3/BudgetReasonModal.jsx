...
const BudgetReasonModal = ({ isOpen, onClose, campaign, reason }) => {
  ...
  const getPeriodicAnalysisInsights = () => {
    ...
    if (p1.budgetUtilization > 120) {
      insights.push({ icon: <AlertCircle className="text-amber-500" />, text: `Short-term budget overspending risk: Budget utilization in the last 24 hours reached ${p1.budgetUtilization.toFixed(1)}%, significantly higher than the 14-day average, recommend monitoring spend rate.` })
    } else if (p1.budgetUtilization < 80) {
      insights.push({ icon: <Info className="text-blue-500" />, text: `Insufficient budget spend: Last 1 day spend only accounts for ${p1.budgetUtilization.toFixed(1)}% of budget, possibly affected by audience saturation or competitive environment.` })
    } else {
      insights.push({ icon: <CheckCircle className="text-green-500" />, text: `Healthy budget utilization: Spend pace highly aligned with budget setting, utilization maintained in the golden range of 90%-110%.` })
    }
    ...
    if (p1.regRate > p7.regRate) {
      insights.push({ icon: <Zap className="text-blue-500" />, text: `Funnel efficiency optimization: Registration conversion rate (CVR1) showed stepwise growth, increasing from 7-day average of ${p7.regRate.toFixed(2)}% to today's ${p1.regRate.toFixed(2)}%.` })
    }
    ...
    insights.push({ icon: <ShieldCheck size={14} className="text-green-600" />, text: "Anomaly monitoring: Core metrics did not trigger circuit breaker threshold, system running smoothly, no abnormal performance due to media policy or API jitter." })
            ...
            <div className="space-y-12 pb-32">
              ...
              <section className="space-y-6">
                <div className="bg-slate-900 rounded-2xl p-6 text-white shadow-xl relative overflow-hidden">
                  <div className="flex items-center gap-2 mb-4"><Zap className="text-yellow-400" size={18} /><h3 className="font-bold">AI Decision Recommendation</h3></div>
                  <p className="text-slate-300 text-sm leading-relaxed mb-6">{reason.detailedReason || reason.reasons?.join(' ')}</p>
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="bg-white/10 p-3 rounded-xl border border-white/5">
                      ...
                      <p className="text-xl font-black text-blue-400">{reason.metrics?.change || '12.5%'}</p>
                    </div>
                  </div>
                </div>

                <div className="space-y-4">
                  <h4 className="text-xs font-black text-slate-400 tracking-widest flex items-center gap-2"><Edit3 size={14} className="text-blue-500" /> Last 14 Days Optimization History</h4>
                ...
                <div className="border border-slate-100 rounded-xl overflow-hidden shadow-sm">
                  <button onClick={() => setShowRawTable(!showRawTable)} className="w-full px-4 py-3 bg-slate-50 flex items-center justify-between hover:bg-slate-100 transition-all">
                    <span className="flex items-center gap-2 text-xs font-bold text-slate-600"><Table size={14}/> Raw Data Details (Last 14 Days)</span>
              ...
              <section>
                <h4 className="text-sm font-bold text-slate-800 mb-4 flex items-center gap-2"><BarChart3 size={16} className="text-blue-500" /> Multi-Dimensional Time Series Analysis</h4>
                <div className="border border-slate-100 rounded-2xl overflow-hidden bg-white shadow-sm mb-4">
                  <table className="w-full text-xs text-left">
                    <thead className="bg-slate-50/80 text-slate-400 font-black uppercase tracking-widest border-b border-slate-100">
                      <tr><th className="p-3 border-r border-slate-100">Dimension</th><th className="p-3">Last 14 Days</th><th className="p-3">Last 7 Days</th><th className="p-3">Last 3 Days</th><th className="p-3">Last 1 Day</th></tr>
                    </thead>
                    <tbody className="divide-y divide-slate-50">
                ...
                <div className="bg-blue-50/40 border border-blue-100 rounded-2xl p-5 shadow-sm">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h5 className="text-xs font-black text-blue-700 tracking-widest">AI Periodic Data Performance Insights</h5>
                  </div>
              ...
              <section>
                <h4 className="text-sm font-bold text-slate-800 mb-4 flex items-center gap-2"><Activity size={16} className="text-blue-500" /> 14-Day Key Metrics Trend (Detailed Dashboard)</h4>
                    ...
                    <div key={idx} className="bg-slate-50 border border-slate-100 rounded-xl p-3 flex flex-col justify-between">
                      ...
                      <div className="space-y-1">
                        <div className="flex justify-between items-baseline">
                          <span className="text-[8px] text-slate-400">Average:</span>
                          <span className={`text-xs font-bold text-slate-700`}>{item.prefix}{item.data.avg.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between items-baseline">
                          <span className="text-[8px] text-slate-400">Slope:</span>
