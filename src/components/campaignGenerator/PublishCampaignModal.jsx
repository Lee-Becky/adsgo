      ...
      <div className="space-y-4">
        ...
        <div className="relative overflow-hidden group bg-white rounded-2xl border border-slate-100 shadow-sm flex items-center transition-all h-16 hover:border-indigo-100">
          ...
          <div className="h-full shrink-0 flex items-center pr-4">
            {platforms.meta.connected ? (
              <button onClick={() => handleDisconnect('meta')} className="px-6 py-2 text-rose-500 text-xs font-black hover:bg-rose-50 rounded-xl transition-colors flex items-center gap-2"><Link2Off size={14} /> Disconnect</button>
            ) : (
              <button onClick={() => handleConnect('meta')} disabled={!!isConnecting} className="px-8 py-2.5 bg-slate-900 text-white text-xs font-black rounded-xl hover:bg-black transition-all disabled:opacity-50 shadow-lg shadow-slate-200">{isConnecting === 'meta' ? <Loader2 size={14} className="animate-spin" /> : 'Connect'}</button>
            )}
          </div>
          {isConnecting === 'meta' && (
    ...
    return (
      <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
        <div className="bg-slate-50 rounded-2xl p-6 space-y-6">
          ...
          {!selections.adAccount && selections.adAccount && (<div className="flex items-center gap-2 p-3 bg-indigo-50 text-indigo-600 rounded-xl text-[10px] font-bold animate-pulse"><AlertCircle size={14} />Please complete all required selections to proceed</div>)}
        </div>
      </div>
    );
  };
    ...
    <div className="fixed inset-0 flex items-center justify-center px-4 animate-in fade-in duration-300" style={{ zIndex: zIndex + 100 }}>
      ...
      <div className="relative bg-white w-full max-w-md rounded-[2rem] shadow-2xl flex flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-300 overflow-hidden p-10">
        ...
        <div className="flex flex-col items-center text-center space-y-6 pt-4">
          ...
          <div className="space-y-3">
            <h3 className="text-lg font-black text-slate-900 tracking-tight">Setting up your dedicated ad account</h3>
            <p className="text-sm font-medium text-slate-600 leading-relaxed">
              An advertising specialist will contact you at your registered email address shortly; please check your email. You can republish from the <button onClick={() => { onClose(); window.location.href = '/autoRegeneration'; }} className="text-indigo-600 hover:text-indigo-700 underline transition-colors bg-transparent border-0 p-0 cursor-pointer">Draft & Recom.</button> page.
            </p>
            <p className="text-xs font-bold text-slate-500">
              Contact us at<br/><a href="mailto:support@adsgo.ai" className="text-indigo-600 hover:text-indigo-700 transition-colors">support@adsgo.ai</a> for real-time updates
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="px-10 pt-10 pb-6 flex items-start justify-between shrink-0">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <span className={`px-3 py-1 rounded-full text-[9px] font-black tracking-widest ${step === 1 ? 'bg-amber-50 text-amber-600' : 'bg-emerald-50 text-emerald-600'}`}>Step {step} of 2</span>
                <div className="flex gap-1">{[1, 2].map((i) => (<div key={i} className={`h-1 rounded-full transition-all duration-500 ${i < step ? 'w-4 bg-emerald-500' : i === step ? 'w-8 bg-slate-900' : 'w-2 bg-slate-200'}`} />))}</div>
              </div>
              <h2 className="text-2xl font-black text-slate-900 tracking-tight">{step === 1 && 'Publishing status'}{step === 2 && 'Confirm brand optimize goal'}</h2>
            </div>
