    ...
    <div className="w-[520px] space-y-4">
      ...
      <div className="flex gap-4">
        ...
        <div className="flex-1 space-y-1.5">
          <label className="text-sm font-medium text-neutral-700 block">Quantity</label>
          <div className="relative">
            {isMultiTemplate ? (
              <div className="relative group/locktip w-full flex items-center justify-between px-3 py-2 bg-neutral-50 border border-neutral-200 rounded-lg text-sm text-neutral-400 cursor-not-allowed select-none">
                <span>{chatSelectedTemplates.size} Image{chatSelectedTemplates.size > 1 ? 's' : ''}</span>
                <Lock size={13} className="text-neutral-300" />
                <div className="pointer-events-none absolute bottom-full right-0 mb-2 hidden group-hover/locktip:block w-max max-w-[220px] bg-neutral-900/90 text-white text-xs rounded-lg px-2.5 py-1.5 shadow-lg leading-relaxed z-10">
              ...
              <>
                <button
                  ...
                  className={cn(
                    'w-full flex items-center justify-between px-3 py-2 bg-white border rounded-lg text-sm text-neutral-700 hover:border-neutral-400 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all',
                    quantityOpen ? 'border-primary-500 ring-2 ring-primary-500/20' : 'border-neutral-300'
                  )}
                >
                  <span>{chatQuantity} Image{chatQuantity > 1 ? 's' : ''}</span>
                  <ChevronDown size={14} className={cn('text-neutral-400 transition-transform duration-200', quantityOpen && 'rotate-180')} />
                </button>
                {quantityOpen && (
                  <>
                    <div className="fixed inset-0" style={{ zIndex: 49 }} onClick={() => setQuantityOpen(false)} />
                    <div className="absolute left-0 right-0 mt-1 bg-white border border-neutral-200 rounded-lg shadow-lg py-1" style={{ zIndex: 50 }}>
                      {[1, 2, 3, 4].map(val => (
                        <button
                          key={val}
                          ...
                          className={cn(
                            ...
                            chatQuantity === val ? 'bg-primary-50 text-primary-600 font-medium' : 'text-neutral-700 hover:bg-neutral-50'
                          )}
                        >
                          {val} Image{val > 1 ? 's' : ''}
        ...
        {/* Ratio */}
        <div className="flex-1 space-y-1.5">
          <label className="text-sm font-medium text-neutral-700 block">Ratio</label>
          <div className="relative">
            <button
              type="button"
              ...
              className={cn(
                'w-full flex items-center justify-between px-3 py-2 bg-white border rounded-lg text-sm text-neutral-700 hover:border-neutral-400 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all',
                ratioOpen ? 'border-primary-500 ring-2 ring-primary-500/20' : 'border-neutral-300'
              )}
            >
              <span className="flex items-center gap-2">
                <RatioBox v={currentRatio} />
                <span className="font-medium">{currentRatio}</span>
                <span className="text-neutral-400 text-xs">{currentRatioTip}</span>
              </span>
              <ChevronDown size={14} className={cn('text-neutral-400 transition-transform duration-200', ratioOpen && 'rotate-180')} />
            </button>
              ...
              <>
                ...
                <div className="absolute left-0 right-0 mt-1 bg-white border border-neutral-200 rounded-lg shadow-lg py-1" style={{ zIndex: 50 }}>
                  <div className="px-3 pt-1.5 pb-1">
                    <span className="text-xs font-medium text-neutral-400">Commonly Used</span>
                  </div>
                  {CHAT_IMG_RATIO_OPTIONS.map(r => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className={cn('flex-1 text-xs', isSelected ? 'text-primary-400' : 'text-neutral-400')}>{r.tip}</span>
                  ...
                  <div className="relative">
                    <button
                      ...
                      className={cn(
                        'w-full px-3 py-2 text-left text-sm transition-colors flex items-center justify-between',
                        ratioMoreOpen ? 'bg-primary-50 text-primary-600' : 'text-neutral-700 hover:bg-neutral-50'
                      )}
                    >
                      <span className="font-medium">More Ratios</span>
                      <ChevronRight size={13} className={cn('transition-colors', ratioMoreOpen ? 'text-primary-400' : 'text-neutral-400')} />
                    </button>
                    {ratioMoreOpen && (
                      <div
                        className="absolute bottom-0 left-full ml-1.5 w-52 bg-white border border-neutral-200 rounded-lg shadow-lg py-1"
                        style={{ zIndex: 51 }}
                      >
                        <div className="px-3 pt-1.5 pb-1">
                          <span className="text-xs font-medium text-neutral-400">More Ratios</span>
                        </div>
      ...
      <div className="space-y-1.5">
        <div className="flex items-center justify-between">
          <label className="text-sm font-medium text-neutral-700">
            Additional Requirements <span className="text-xs text-neutral-400 font-normal">(Optional)</span>
          </label>
        </div>
        <textarea
          ...
          rows={4}
          className="w-full px-3 py-2 bg-white border border-neutral-300 rounded-lg text-sm placeholder:text-neutral-400 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all resize-none"
        />
        ...
        <div className="flex items-center gap-2 pt-0.5">
          ...
          <button
            ...
            className="p-1 rounded-full text-neutral-400 hover:text-primary-500 hover:bg-primary-50 transition-colors"
        ...
        <div className="flex flex-wrap gap-1.5">
          {pageSuggestions.map(s => (
            <button
              ...
              className="group px-2.5 py-1 rounded-full text-sm font-normal bg-white border border-neutral-200 text-neutral-600 hover:border-primary-300 hover:text-primary-600 hover:bg-primary-50 transition-all inline-flex items-center gap-1.5"
            >
              <span>{s}</span>
              <ArrowUpRight size={12} className="text-neutral-400 group-hover:text-primary-500 transition-colors" />
            </button>
          ))}
        </div>
      </div>
