      ...
      <div className="flex-1 max-w-[1400px] mx-auto w-full p-10 flex gap-12">
        ...
        <aside className="w-72 shrink-0">
          <div className="sticky top-32 space-y-6">
            <nav className="p-3 bg-white border border-slate-100 rounded-[32px] shadow-sm space-y-1">
              {[
                ...
                >
                  <tab.icon size={20} className={activeTab === tab.id ? `text-${tab.color}-500` : 'text-slate-300 group-hover:text-slate-400'} />
        ...
        <main ref={scrollContainerRef} className="flex-1 space-y-16 pb-32">
          ...
          <section ref={sectionRefs.basicInfo} className="bg-white border border-slate-100 rounded-[40px] p-10 shadow-sm space-y-10 scroll-mt-32">
            <div className="flex items-center gap-4 border-b border-slate-50 pb-6">
              <div className="w-12 h-12 rounded-2xl bg-indigo-50 flex items-center justify-center text-indigo-600 shadow-sm">
                <Info size={24} />
              </div>
              <div>
                <h4 className="text-xl font-black text-slate-900">Basic Info</h4>
                <p className="text-xs text-slate-400 font-bold">Core details that help us describe and highlight your product.</p>
              </div>
            </div>
            <div className="space-y-8">
              <div className="grid grid-cols-2 gap-10">
                <div className="space-y-3">
                  ...
                  <input type="text" value={productForm.name || ''} onChange={(e) => updateForm('name', e.target.value)} className="w-full bg-white border border-slate-200 rounded-2xl px-6 py-4 text-sm font-bold text-slate-700 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/5 focus:outline-none transition-all shadow-sm" />
                </div>
                <div className="space-y-3">
                  <label className="text-[11px] font-black text-slate-400 flex items-center gap-2">
                    <div className="w-1 h-3 bg-indigo-400 rounded-full" />
                  ...
                  <input type="text" value={productForm.url || ''} onChange={(e) => updateForm('url', e.target.value)} placeholder="https://example.com/product" className="w-full bg-white border border-slate-200 rounded-2xl px-6 py-4 text-sm font-bold text-slate-700 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/5 focus:outline-none transition-all shadow-sm" />
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="relative">
                  <textarea className="w-full bg-white border border-slate-200 rounded-3xl px-6 py-5 text-sm font-bold text-slate-700 min-h-[160px] resize-none focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/5 outline-none transition-all shadow-sm" value={productForm.description || ''} onChange={(e) => updateForm('description', e.target.value)} />
                  <div className="absolute bottom-4 right-6 px-3 py-1 bg-slate-50 border border-slate-100 rounded-lg"><span className="text-[10px] text-slate-400 font-black">{(productForm.description || '').length}/5000</span></div>
                </div>
              </div>
            </div>
          </section>

          <section ref={sectionRefs.usps} className="bg-white border border-slate-100 rounded-[40px] p-10 shadow-sm space-y-10 scroll-mt-32">
            <div className="flex items-center justify-between border-b border-slate-50 pb-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h4 className="text-xl font-black text-slate-900">Selling Points (USPs)</h4>
                  <p className="text-xs text-slate-400 font-bold">Highlight the unique benefits of your product.</p>
                </div>
              </div>
              <span className="px-4 py-1 bg-slate-100 rounded-full text-[10px] font-black text-slate-500">{(productForm.usps || []).length}/20</span>
            </div>
            <div className="grid grid-cols-2 gap-6">
              {(productForm.usps || ['']).map((usp, i) => (
                <div key={i} className="flex items-center gap-4">
                  <div className="flex-1 relative group/input">
                    <div className="absolute left-5 top-1/2 -translate-y-1/2 w-7 h-7 rounded-lg bg-slate-100 border border-slate-200 flex items-center justify-center text-[11px] font-black text-slate-400 group-focus-within/input:bg-amber-100 group-focus-within/input:text-amber-600 transition-all">{i + 1}</div>
                    <input type="text" value={usp} onChange={(e) => { const newUsps = [...(productForm.usps || [''])]; newUsps[i] = e.target.value; updateForm('usps', newUsps); }} placeholder="e.g., AI Auto Management" className="w-full bg-white border border-slate-200 rounded-2xl pl-16 pr-6 py-4 text-sm font-bold text-slate-700 focus:border-amber-400 focus:ring-4 focus:ring-amber-500/5 outline-none transition-all shadow-sm" />
                  </div>
                  <button onClick={() => { const newUsps = (productForm.usps || []).filter((_, idx) => idx !== i); updateForm('usps', newUsps.length > 0 ? newUsps : ['']); }} className="p-3 text-slate-300 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-all"><Trash2 size={20} /></button>
                </div>
              ))}
              {(productForm.usps || []).length < 20 && (
                <button onClick={() => updateForm('usps', [...(productForm.usps || []), ''])} className="flex items-center justify-center gap-2 py-4 border-2 border-dashed border-slate-200 rounded-2xl text-slate-400 hover:border-amber-300 hover:text-amber-600 hover:bg-amber-50/30 transition-all font-black text-xs shadow-sm"><Plus size={18} strokeWidth={3} /> Add Selling Point</button>
              )}
            </div>
          </section>

          <section ref={sectionRefs.positioning} className="bg-white border border-slate-100 rounded-[40px] p-10 shadow-sm space-y-10 scroll-mt-32">
            <div className="flex items-center gap-4 border-b border-slate-50 pb-6">
              <div className="w-12 h-12 rounded-2xl bg-purple-50 flex items-center justify-center text-purple-600 shadow-sm">
                <Target size={24} />
              </div>
              <div>
                <h4 className="text-xl font-black text-slate-900">Product Positioning</h4>
                <p className="text-xs text-slate-400 font-bold">Define your market stance and core value.</p>
              </div>
            </div>
            <div className="space-y-10">
              <div className="space-y-3">
                <label className="text-[11px] font-black text-slate-400 flex items-center gap-2"><div className="w-1 h-3 bg-purple-400 rounded-full" />Value Proposition</label>
              ...
              <div className="grid grid-cols-2 gap-10">
                <div className="space-y-3">
                  <label className="text-[11px] font-black text-slate-400 flex items-center gap-2"><div className="w-1 h-3 bg-purple-400 rounded-full" />Feature Keywords</label>
                ...
                <div className="space-y-3">
                  <label className="text-[11px] font-black text-slate-400 flex items-center gap-2"><div className="w-1 h-3 bg-purple-400 rounded-full" />Usage Scenarios</label>
              ...
              <div className="grid grid-cols-2 gap-10">
                <div className="space-y-3">
                  <label className="text-[11px] font-black text-slate-400 flex items-center gap-2"><div className="w-1 h-3 bg-purple-400 rounded-full" />Pain Points</label>
                ...
                <div className="space-y-3">
                  <label className="text-[11px] font-black text-slate-400 flex items-center gap-2"><div className="w-1 h-3 bg-purple-400 rounded-full" />Buying Motivations</label>
          ...
          <section ref={sectionRefs.audience} className="bg-white border border-slate-100 rounded-[40px] p-10 shadow-sm space-y-10 scroll-mt-32">
            <div className="flex items-center gap-4 border-b border-slate-50 pb-6">
              <div className="w-12 h-12 rounded-2xl bg-emerald-50 flex items-center justify-center text-emerald-600 shadow-sm">
                <User size={24} />
              </div>
              <div>
                <h4 className="text-xl font-black text-slate-900">Audience Profile</h4>
                <p className="text-xs text-slate-400 font-bold">Describe your ideal target customers.</p>
              </div>
            </div>
            <div className="space-y-10">
              {(productForm.audience || []).map((profile, idx) => (
                <div key={profile.id} className="p-10 bg-slate-50/50 border border-slate-100 rounded-[40px] space-y-8 group transition-all hover:bg-white hover:shadow-xl hover:border-emerald-100">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-5">
                      <div className="w-14 h-14 rounded-[20px] bg-emerald-100 flex items-center justify-center text-emerald-600 shadow-inner"><User size={28} strokeWidth={2.5} /></div>
                      <div>
                        <div className="flex items-center gap-2">
                          <input type="text" value={profile.name || ''} onChange={(e) => { const newAudience = (productForm.audience || []).map(a => a.id === profile.id ? { ...a, name: e.target.value } : a); updateForm('audience', newAudience); }} className="bg-transparent border-none p-0 text-xl font-black text-slate-900 focus:ring-0 outline-none w-auto min-w-[150px]" />
                          <Edit2 size={16} className="text-slate-300 group-hover:text-emerald-500 transition-colors" />
                        </div>
                        <p className="text-xs text-slate-400 font-bold">Set specific targeting rules</p>
                      </div>
                    </div>
                    <button onClick={() => { const newAudience = (productForm.audience || []).filter(a => a.id !== profile.id); updateForm('audience', newAudience.length > 0 ? newAudience : [{ id: Date.now(), name: 'Audience Name', age: '', gender: 'All', traits: [] }]); }} className="p-3 text-slate-300 hover:text-rose-500 hover:bg-rose-50 rounded-2xl transition-all opacity-0 group-hover:opacity-100"><Trash2 size={20} /></button>
                  </div>
                  <div className="grid grid-cols-2 gap-10">
                    <div className="space-y-3">
                      <label className="text-[11px] font-black text-slate-400 flex items-center gap-2"><div className="w-1 h-3 bg-emerald-400 rounded-full" />Age Range</label>
                      <input type="text" value={profile.age || ''} onChange={(e) => { const newAudience = (productForm.audience || []).map(a => a.id === profile.id ? { ...a, age: e.target.value } : a); updateForm('audience', newAudience); }} placeholder="e.g. 25-45" className="w-full bg-white border border-slate-200 rounded-2xl px-6 py-4 text-sm font-bold text-slate-700 focus:border-emerald-400 focus:ring-4 focus:ring-emerald-500/5 outline-none shadow-sm transition-all" />
                    </div>
                    <div className="space-y-3">
                      <label className="text-[11px] font-black text-slate-400 flex items-center gap-2"><div className="w-1 h-3 bg-emerald-400 rounded-full" />Gender</label>
                  ...
                  <div className="space-y-3">
                    <label className="text-[11px] font-black text-slate-400 flex items-center gap-2"><div className="w-1 h-3 bg-emerald-400 rounded-full" />Interests / Traits</label>
              ...
              <button onClick={() => updateForm('audience', [...(productForm.audience || []), { id: Date.now(), name: 'Audience Name', age: '', gender: 'All', traits: [] }])} className="w-full py-5 border-2 border-dashed border-slate-200 rounded-[32px] text-slate-400 hover:border-emerald-300 hover:text-emerald-600 hover:bg-emerald-50/30 transition-all font-black text-sm flex items-center justify-center gap-2 shadow-sm"><Plus size={20} strokeWidth={3} /> Add Audience Profile</button>
            </div>
          </section>

          <section ref={sectionRefs.assets} className="bg-white border border-slate-100 rounded-[40px] p-10 shadow-sm space-y-12 scroll-mt-32">
            <div className="flex items-center gap-4 border-b border-slate-50 pb-6">
              <div className="w-12 h-12 rounded-2xl bg-blue-50 flex items-center justify-center text-blue-600 shadow-sm">
                <Layers size={24} />
              </div>
              <div>
                <h4 className="text-xl font-black text-slate-900">Product Assets</h4>
                <p className="text-xs text-slate-400 font-bold">Manage images and videos for your ad creative.</p>
              </div>
            </div>
