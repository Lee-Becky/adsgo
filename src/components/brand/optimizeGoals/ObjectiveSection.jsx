...
const ObjectiveSection = ({ formData, updateFormData, validation, setValidation }) => {
  ...
  return (
    <div className="bg-white rounded-2xl border border-neutral-200 shadow-[0_4px_24px_rgba(0,0,0,0.04)] relative z-[60] animate-in fade-in duration-700">
      <header className="px-10 py-6 bg-neutral-100 border-b border-neutral-200 flex items-center justify-between rounded-t-2xl">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-sm font-black text-neutral-900">Promote Objective <span className="text-rose-500 ml-1">*</span></h2>
          </div>
      ...
      <div className="p-10 space-y-12">
              ...
              return (
                <button
                  ...
                  className={`group relative flex flex-col items-center justify-center p-6 rounded-2xl border-2 transition-all duration-300 ${
                    isActive 
                      ? 'border-primary-600 bg-neutral-900 text-white shadow-xl scale-105 z-10' 
                      : 'border-neutral-50 bg-neutral-50/50 text-neutral-400 hover:border-primary-200 hover:bg-white hover:text-neutral-600'
                  }`}
                >
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-300 mb-4 shadow-sm ${
                    isActive ? 'bg-primary-500 text-white shadow-lg' : `${obj.bg} ${obj.color}`
                  }`}>
                    <Icon size={28} strokeWidth={isActive ? 2.5 : 2} />
                  </div>
                  <div className="text-center">
                    <p className={`text-xs font-black leading-tight mb-1 ${isActive ? 'text-white' : 'text-neutral-900'}`}>
                      {obj.label}
                    </p>
                    <p className={`text-[10px] font-medium leading-tight ${isActive ? 'text-primary-200' : 'text-neutral-400'}`}>
                      {obj.description}
                    </p>
                  </div>
                </button>
        ...
        {formData.campaignObjective && (
          <div className="space-y-8 animate-in slide-in-from-top-4 duration-500 pt-8 border-t border-neutral-100 flex flex-col lg:flex-row lg:items-center gap-10">
            <div className="lg:w-[35%] space-y-4">
              <div className="flex items-center gap-2">
                <h3 className="text-xs font-black text-neutral-900">Conversion Event</h3>
              </div>
            ...
            <div className="flex-1">
              <div className="relative z-[100] max-w-lg">
                <div 
                  ...
                  className={`w-full flex items-center justify-between px-6 py-4 h-16 bg-neutral-50 border-2 rounded-2xl cursor-pointer transition-all ${
                    formData.adsetGoal ? 'border-primary-500 bg-white shadow-lg' : 'border-neutral-100 hover:border-primary-100 hover:bg-white'
                  }`}
                >
                  <div className="flex items-center gap-4">
                    {formData.adsetGoal ? (
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-black text-neutral-900">{currentGoal?.label}</span>
                          {formData.event && (
                            <>
                              <ChevronRight size={12} className="text-neutral-300" />
                              <span className="text-sm font-black text-primary-600">{formData.event}</span>
                            </>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-bold text-neutral-300">Choose event or outcome...</span>
                      </div>
                    )}
                  </div>
                  <ChevronRight size={20} className={`text-neutral-300 transition-transform duration-300 ${isEventSelectOpen ? 'rotate-90 text-primary-500' : ''}`} />
                </div>

                {isEventSelectOpen && (
                  <div className="absolute z-[110] mt-3 w-full bg-white border border-neutral-100 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] p-4 animate-in fade-in zoom-in-95 duration-200">
                    
                    {selectorStage === 'goal' ? (
                      <div className="space-y-3">
                        <p className="text-[10px] font-black text-neutral-400 mb-2 px-2">Select conv. goal</p>
                          ...
                          {adsetGoals.map(goal => (
                            <button
                              ...
                              className={`w-full text-left px-4 py-3.5 rounded-xl text-[13px] font-bold transition-all flex items-center justify-between group ${
                                formData.adsetGoal === goal.value ? 'bg-neutral-900 text-white' : 'hover:bg-neutral-50 text-neutral-600'
                              }`}
                            >
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center gap-2 px-1">
                          <button onClick={() => setSelectorStage('goal')} className="p-1 hover:bg-neutral-100 rounded-md transition-colors text-neutral-400 hover:text-neutral-900">
                            <ChevronLeft size={16} />
                          </button>
                          <p className="text-[10px] font-black text-neutral-400">Back</p>
                        </div>
                        <div className="relative">
                          <Search size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-neutral-300" />
                          <input 
                            className="w-full pl-11 pr-4 py-3 bg-neutral-50 border-none rounded-xl text-sm font-bold text-neutral-900 focus:ring-2 focus:ring-primary-500/20"
                          ...
                          {filteredEvents.map(ev => (
                            <button
                              ...
                              className={`w-full text-left px-4 py-3 rounded-xl text-[13px] font-bold transition-all flex items-center justify-between group ${
                                formData.event === ev ? 'bg-primary-600 text-white shadow-lg shadow-primary-200' : 'hover:bg-neutral-50 text-neutral-600'
                              }`}
                            >
                              {ev}
