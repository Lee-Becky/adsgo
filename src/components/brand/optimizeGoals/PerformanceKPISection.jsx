...
const PerformanceKPISection = ({ formData, updateFormData, updateFormDataDeep, validation, setValidation }) => {
  ...
  useEffect(() => {
    ...
    setValidation(prev => ({ ...prev, kpi: kpiValid }))
  }, [formData.kpiMode, formData.unifiedKPI, formData.targetLocations, formData.splitKPIs, setValidation])
        ...
        <div className="space-y-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <h3 className="text-sm font-bold text-slate-500">KPI Type</h3>
              ...
              <div className="flex p-1 bg-slate-50 rounded-xl border border-slate-200">
                <button
                  ...
                  className={`px-4 py-1.5 rounded-lg text-[10px] font-black transition-all ${
                    formData.kpiMode === 'unified' ? 'bg-white text-slate-900 shadow-sm border border-slate-100' : 'text-slate-400'
                  }`}
                >
                ...
                <button
                  ...
                  className={`px-4 py-1.5 rounded-lg text-[10px] font-black transition-all ${
                    formData.kpiMode === 'split' ? 'bg-white text-slate-900 shadow-sm border border-slate-100' : 'text-slate-400'
          ...
          {formData.kpiMode === 'unified' ? (
            <div className="space-y-4 max-w-md">
              <span className="text-sm font-bold text-slate-500 px-1">Target KPI</span>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100 shadow-inner">
                ...
                />
                <button onClick={applyBatchKPI} className="px-4 py-2 bg-slate-900 text-white rounded-xl text-[10px] font-black hover:bg-black transition-all">Apply</button>
              </div>
                ...
                {formData.targetLocations.map(loc => (
                  <div key={loc.value} className="p-4 bg-slate-50 border border-slate-100 rounded-2xl flex items-center justify-between group hover:bg-white hover:border-indigo-200 transition-all">
                    ...
                    <div className="relative w-24 text-right">
                      <span className="text-[10px] font-black text-slate-400 mr-1">{formData.kpiType}</span>
                      <input
                        type="number"
