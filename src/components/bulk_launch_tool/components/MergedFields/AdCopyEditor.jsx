    ...
    >
      <div className="bg-white rounded-2xl shadow-2xl w-[560px] max-w-[92vw] max-h-[90vh] overflow-hidden flex flex-col animate-in zoom-in-95">
        <div className="flex items-center justify-between px-5 py-4 border-b border-neutral-100 shrink-0">
          ...
          <button onClick={onClose} className="text-neutral-400 hover:text-neutral-700 p-1 rounded-base hover:bg-neutral-50 shrink-0">
            <X size={16} />
          </button>
        </div>
        ...
        <div className="px-5 py-5 space-y-5 overflow-y-auto flex-1">
          {isMeta ? (
            <>
              ...
              <div className="flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-medium text-neutral-700">
                    广告标题 <span className="text-neutral-400 font-normal">（≤ {META_TITLE_LEN} 字符 · 最多 {META_MAX_TITLES} 个）</span>
                  </label>
                  <span className="text-[10px] text-neutral-400">{(draft.titles || []).length} / {META_MAX_TITLES}</span>
                </div>
                {(draft.titles || []).map((t, i) => (
                  <div key={i} className="flex items-start gap-1.5">
                    <span className="text-[10px] font-bold text-neutral-400 mt-2.5 w-4 text-center shrink-0 tabular-nums">{i + 1}</span>
                    ...
                    <button
                      ...
                      className="w-7 h-7 mt-1 flex items-center justify-center rounded-full text-neutral-400 hover:text-rose-500 hover:bg-rose-50 disabled:text-neutral-200 disabled:hover:bg-transparent disabled:cursor-not-allowed transition-colors shrink-0"
                    >
                      <Trash2 size={12} />
                    </button>
                  </div>
                ...
                <button
                  ...
                  className="self-start inline-flex items-center gap-1 text-xs font-medium text-primary-600 hover:text-primary-700 hover:bg-primary-50 disabled:text-neutral-300 disabled:hover:bg-transparent disabled:cursor-not-allowed px-2 py-1 rounded-base transition-colors"
                >
                  <Plus size={12} /> 添加标题
                </button>
              </div>
              ...
              <div className="flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-medium text-neutral-700">
                    广告正文 <span className="text-neutral-400 font-normal">（≤ {META_BODY_LEN} 字符 · 最多 {META_MAX_BODIES} 个）</span>
                  </label>
                  <span className="text-[10px] text-neutral-400">{(draft.bodies || []).length} / {META_MAX_BODIES}</span>
                </div>
                  ...
                  <div key={i} className="flex items-start gap-1.5">
                    ...
                    <div className="flex-1 min-w-0 relative">
                      <textarea
                        ...
                        className="w-full px-3 py-2 text-sm bg-white border border-neutral-200 rounded-base outline-none resize-y hover:border-neutral-300 focus:border-primary-500 focus:shadow-primary-focus transition-all"
                      />
                      {enableProductVariables && (
            ...
            <>
              ...
              <div className="flex flex-col gap-1.5">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-medium text-neutral-700">广告文案 <span className="text-neutral-400 font-normal">（≤ {TIKTOK_TEXT_LEN} 字符）</span></label>
                  {enableProductVariables && (
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-medium text-neutral-700">落地页 URL</label>
            <TextInput
              ...
              type="url"
            />
            <p className="text-[10px] text-neutral-400 px-1">留空 → 发布时回退到产品默认落地页</p>
          </div>

          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-medium text-neutral-700">行动号召按钮（CTA）</label>
            <Select
        ...
        <div className="px-5 py-3 bg-neutral-50/50 border-t border-neutral-100 flex items-center justify-end gap-2 shrink-0">
