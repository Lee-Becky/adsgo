  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
      <button
        type="button"
        onClick={() => setOpen(o => !o)}
        className="w-full flex items-center gap-3 px-6 py-4 hover:bg-gray-50/40 transition-colors"
      >
        ...
        <div className="flex-1 min-w-0 text-left">
          <h3 className="text-base font-semibold text-gray-900 leading-tight">高级设置</h3>
          <p className="text-xs text-gray-500 mt-0.5">
            可选 <span className="font-semibold text-gray-700">{total}</span> 项 ·
      ...
      {open && (
        <div className="border-t border-gray-100">
          {/* segmented pill tabs */}
          <div className="px-6 py-3 bg-gray-50/40 border-b border-gray-100">
            <div className="inline-flex p-1 bg-white rounded-full border border-gray-100 shadow-sm">
              {tabs.map(t => {
                ...
                return (
                  <button
                    ...
                    className={[
                      ...
                      active ? 'bg-primary-500 text-white shadow' : 'text-gray-600 hover:text-gray-800',
                    ].join(' ')}
                  >
                    <span>{t.label}</span>
                    <span className={[
          ...
          {/* fields */}
          <div className="px-6 py-5 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-5 bg-gray-50/30">
            {activeDefs.length === 0 ? (
              <p className="text-xs text-gray-400 py-2 col-span-full">该层级无可选字段。</p>
            ) : activeDefs.map(def => (
