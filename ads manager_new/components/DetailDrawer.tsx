...
import { Campaign, CampaignMetrics, DailyData } from '../types';
...
const DetailDrawer: React.FC<DetailDrawerProps> = ({ campaign, isOpen, onClose }) => {
  ...
  // 趋势计算函数：线性回归 (y = mx + b)
  const calculateLinearRegression = (data: any[], key: string) => {
    const n = data.length;
  ...
  const calculatePeriodStats = (days: number) => {
    ...
    const sum = (key: keyof CampaignMetrics) => periodData.reduce((acc, d) => acc + (d[key] || 0), 0);
  ...
  const getPeriodicAnalysisInsights = () => {
    ...
    // 5. 异常监测 (从表格移动到透视表)
    // Fix: Added ShieldCheck to the imports above to resolve the "Cannot find name 'ShieldCheck'" error.
    insights.push({ icon: <ShieldCheck size={14} className="text-green-600" />, text: "异常监测：核心指标未触发熔断阈值，系统运行平稳，无由于媒体政策或 API 抖动导致的异常表现。" });

    return insights.slice(0, 5); 
  };

  const getFunnelMetrics = (data: DailyData[]) => {
    const sum = (key: keyof CampaignMetrics) => data.reduce((acc, d) => acc + (d[key] || 0), 0);
  ...
  const compare = (val: number, bench: number, lowerIsBetter = false) => {
  ...
  const comparisonRows = [
    { l: '总消耗（日均消耗）', f: (p: any) => `¥${p.spendTotal.toFixed(0)} (${p.spendAvg.toFixed(0)})` },
    { l: '总转化量（日均转化量）', f: (p: any) => `${p.convTotal.toFixed(0)} (${p.convAvg.toFixed(0)})` },
    { l: 'KPI达成值（CPA或ROAS）', f: (p: any) => `¥${p.cpa.toFixed(1)}` },
    { l: 'KPI达成率', f: (p: any) => `${p.cpaAchievement.toFixed(0)}%` },
    { l: 'KPI达成排名', f: (p: any) => `${p.rank}/12 (前${((p.rank / 12) * 100).toFixed(0)}%)` },
    { l: 'CTR（vs均值）', f: (p: any) => `${p.ctr.toFixed(2)}% (vs ${periods['14d'].ctr.toFixed(2)}%)` },
    { l: 'KPI CVR（vs均值）', f: (p: any) => `${p.regRate.toFixed(2)}% (vs ${periods['14d'].regRate.toFixed(2)}%)` },
  ];
    ...
    <>
      ...
      <div className={`fixed top-0 right-0 z-50 h-full w-full max-w-2xl bg-white shadow-2xl transform transition-transform duration-500 ease-out flex flex-col ${isVisible ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="flex-grow overflow-y-auto p-6 space-y-12 pb-32">
          ...
          {/* Section 4: 14-day Trend - Aesthetic Refinement */}
          <section>
